...
final_exp_name="/anvil/projects/x-cis251123/aperdomo/results/701515_split_250_recovery/COO/lr_$lr/max_genome_$MAX_GENOME/island_$NUM_ISLAND/$FOLDER"
...
cp "$DATAPATH/COO_train.csv" "$LOCAL_DATA/"
cp "$DATAPATH/COO_val.csv" "$LOCAL_DATA/"
...
time srun -n "$SLURM_NTASKS" --cpu-bind=cores "$EXAMM/build/mpi/examm_mpi" \
    --training_filenames "$LOCAL_DATA/COO_train.csv" \
    --test_filenames "$LOCAL_DATA/COO_val.csv" \
    --time_offset $offset \
