...
final_exp_name="/anvil/projects/x-cis251123/aperdomo/results/701515_split_250_recovery/ED/lr_$lr/max_genome_$MAX_GENOME/island_$NUM_ISLAND/$FOLDER"
...
cp "$DATAPATH/ED_train.csv" "$LOCAL_DATA/"
cp "$DATAPATH/ED_val.csv" "$LOCAL_DATA/"
...
time srun -n "$SLURM_NTASKS" --cpu-bind=cores "$EXAMM/build/mpi/examm_mpi" \
    --training_filenames "$LOCAL_DATA/ED_train.csv" \
    --test_filenames "$LOCAL_DATA/ED_val.csv" \
    --time_offset $offset \
