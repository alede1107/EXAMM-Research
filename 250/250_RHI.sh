...
do
    exp_name="/anvil/scratch/x-aperdomo/results/701515_split_250/RHI/lr_$lr/max_genome_$MAX_GENOME/island_$NUM_ISLAND/$folder"
    mkdir -p "$exp_name"
    echo "Iteration: $exp_name"
    echo "###-------------------###"

    time srun "$EXAMM/build/mpi/examm_mpi" \
        --training_filenames "$DATAPATH/RHI_train.csv" \
        --test_filenames "$DATAPATH/RHI_val.csv" \
        --time_offset $offset \
